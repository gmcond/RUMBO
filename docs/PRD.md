...
Decisión fijada (16/07/2026): dirección de arte **«Travesía»** (mezcla de las propuestas Bitácora × Estela) — claro papel crema `#F9F5EC` / tinta marina `#1B2C3A` / primario petróleo `#15516C` / acento naranja salvavidas `#D8451B`; oscuro azul noche `#0E1D2B` con texto crema. Tipografía: Fraunces 600 (titulares) + Karla (UI). Wordmark: «RUMB + O-aguja» en Bricolage Grotesque 800 (subset), variante A del laboratorio; la rosa de los vientos completa se reserva como asset decorativo. Tokens centralizados en `app/globals.css`.
